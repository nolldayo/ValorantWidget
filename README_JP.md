...
## 開発ポリシー
riotの「当該利用者の同意なしに対戦データなどを使用するべきではない」とのAPIポリシーを遵守し、  
本ツールでは利用者以外のマッチング履歴、また進行中のマッチングのデータ取得は行いません。  
riotよりツールの提供停止要望が来た場合、可能な限り速やかにプロジェクトは消去されます。  
コミュニティの活性化を希望しています。riotが現在著しく限定的な範囲で提供しているAPIが広く一般開発者に提供されること希望します。  

## アップデート情報
|  日時  |  内容  |
| ---- | ---- |
|  2021.08.24  |  自動W/L記録機能の追加  |
...
[PHPが導入されていないというエラーで起動出来ないとき](https://twitter.com/YNZjp/status/1429255363719622659)  
...
**6.W/L表示用htmlファイルをOBSに設定する**  
W/L表示については、表示したくないという方はこの項目をスキップしてください。  
上記のアウトプット用htmlの登録と同じように、ブラウザソースからoutputWL.htmlを追加してください。  
入力サイズは　**幅150、高さ40**です。  
W/Lは、「#ValorantWidget.bat」が起動される度に初期化されます。  
すなわち、配信を始めるときに毎回再起動を行えばプログラムは配信中のみのW/Lを自動で記録し、表示します。  
